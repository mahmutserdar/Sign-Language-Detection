...
#data = pd.read_excel("../featureExtraction/asl_alphabet_data.xlsx", header=0)
data.pop("CHARACTER")
...
for fold, (trainIndex, valIndex) in enumerate(kf.split(coordinates)):
    ...
    for epochi in range(1, epoch+1):
        ...
        trainPrec = precision_score(groupValueTraining.cpu(), torch.argmax(trainOutput, dim=1), average='weighted', zero_division=0)
        trainRec = recall_score(groupValueTraining.cpu(), torch.argmax(trainOutput, dim=1), average='weighted', zero_division=0)
        trainF1 = f1_score(groupValueTraining.cpu(), torch.argmax(trainOutput, dim=1), average='weighted', zero_division=0)

        # Calculate metrics for validation data
        valPrec = precision_score(groupValueValidation.cpu(), torch.argmax(outputValid, dim=1), average='weighted', zero_division=0)
        valRec = recall_score(groupValueValidation.cpu(), torch.argmax(outputValid, dim=1), average='weighted', zero_division=0)
        valF1 = f1_score(groupValueValidation.cpu(), torch.argmax(outputValid, dim=1), average='weighted', zero_division=0)
        ...
        if epochi % 10 == 0:
            print(f'Fold: {fold+1}, Epoch : {epochi}, Training Loss: {lossTrain.item()}, Validation Loss: {lossVal.item()}')
...
avgTrainLoss = np.mean(foldTrainLosses, axis=0) #Using axis=0 with these arrays calculates the mean across the same epoch across different folds
...
finalAvgTrainAccuracy = avgTrainAccuracy[-1] # refers to the last element of that list, evaluate the final performance after completing all training epochs
